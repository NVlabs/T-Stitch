...
class MultiTStitch(nn.Module):
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        allocation = torch.tensor(self.fractions)
        torch.cumsum(allocation, dim=0, out=allocation)

        for idx, milestone in enumerate(allocation):
...
class TStitch(nn.Module):
    def __init__(self, denoisers, diffusion_steps=1000, ratio=0.1):
        '''
            :param denoisers: list of denoisers
            :param diffusion_steps: total number of diffusion steps
            :param ratio: ratio of the first denoiser (the smaller one)
        '''
        super(TStitch, self).__init__()
        self.denoisers = nn.ModuleList(denoisers)
        self.ratio = ratio
        self.in_channels = denoisers[0].in_channels
        self.diffusion_steps = diffusion_steps
    ...
    def forward(self, x, t, y):
        idx = 0 if t[0].item()/self.diffusion_steps >= (1 - self.ratio) else 1
